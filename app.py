...
class PointTemplate(db.Model):
    ...
    def to_dict(self):
        return {"id": self.id, "name": self.name, "quantity": self.quantity, "sub_points": [sp.to_dict() for sp in self.sub_points]}
...
def get_panel_point_summary(panel_id):
    ...
    for equip in panel.equipment:
        for selected_point in equip.selected_points:
            ...
            for sub_point in selected_point.sub_points:
                point_type = sub_point.point_type
...
def add_point():
    ...
    new_point = PointTemplate(name=data['name'], quantity=data.get('quantity', 1))
...
def setup_database(app):
    with app.app_context():
        ...
        if PointTemplate.query.first() is None:
            ...
            # Create PointTemplates with SubPointTemplates
            p1 = PointTemplate(name="Supply Air Temp", quantity=1)
            p1.sub_points.append(SubPointTemplate(name="Supply Air Temp", point_type="AI"))
            db.session.add(p1)

            p2 = PointTemplate(name="Temp/Humidity Sensor", quantity=2)
            p2.sub_points.append(SubPointTemplate(name="Temperature", point_type="AI"))
            p2.sub_points.append(SubPointTemplate(name="Humidity", point_type="AI"))
            db.session.add(p2)

            p3 = PointTemplate(name="Filter Status", quantity=1)
            p3.sub_points.append(SubPointTemplate(name="Filter Status", point_type="DI"))
            db.session.add(p3)

            p4 = PointTemplate(name="Fan Status/Control", quantity=2)
            p4.sub_points.append(SubPointTemplate(name="Fan Status", point_type="DI"))
            p4.sub_points.append(SubPointTemplate(name="Fan Control", point_type="DO"))
            db.session.add(p4)
            ...
            et_ahu.available_points.append(EquipmentTemplatePoint(point=p1, quantity=1))
            et_ahu.available_points.append(EquipmentTemplatePoint(point=p3, quantity=1))
            et_ahu.available_points.append(EquipmentTemplatePoint(point=p4, quantity=1))

            et_fcu = EquipmentTemplate(type_key='fcu', name='Fan Coil Unit')
            db.session.add(et_fcu)
            et_fcu.available_points.append(EquipmentTemplatePoint(point=p1, quantity=1))
            et_fcu.available_points.append(EquipmentTemplatePoint(point=p4, quantity=1))

            panel1 = Panel(panel_name="LP-GF-01", floor="Ground Floor")
            db.session.add(panel1)

            db.session.commit()

            se1 = ScheduledEquipment(instance_name="AHU-GF-01", quantity=1, panel_id=panel1.id, equipment_template_id=et_ahu.id)
            se1.selected_points.extend([p1, p3, p4])
            db.session.add(se1)
